          ...
          void updateGesture(GestureMap next) {
            if (!validateGestureMap(next)) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text(
                    'Keep at least one gesture set to Show menu.',
                  ),
                ),
              );
              return;
            ...
            child: SizedBox(
              ...
              child: Column(
                children: [
                  ...
                  Expanded(
                    child: ListView(
                      ...
                      children: [
                Text('Tap zones', style: Theme.of(context).textTheme.titleSmall),
                ...
                Wrap(
                  ...
                  children: PageTurnTapMode.values.map((mode) {
                    ...
                    return ChoiceChip(
                      ...
                      selected: selected,
                      onSelected: (_) => update(current.copyWith(tapMode: mode)),
                    );
                  }).toList(),
                ),
                const Divider(height: 32),
                Text('Swipe', style: Theme.of(context).textTheme.titleSmall),
                SwitchListTile(
                  title: const Text('Swipe left → next'),
                  value: current.swipeLeft,
                  onChanged: (v) => update(current.copyWith(swipeLeft: v)),
                ),
                SwitchListTile(
                  title: const Text('Swipe right → previous'),
                  value: current.swipeRight,
                  onChanged: (v) => update(current.copyWith(swipeRight: v)),
                ),
                SwitchListTile(
                  title: const Text('Swipe up → next'),
                  value: current.swipeUp,
                  onChanged: (v) => update(current.copyWith(swipeUp: v)),
                ),
                SwitchListTile(
                  title: const Text('Swipe down → previous'),
                  value: current.swipeDown,
                  onChanged: (v) => update(current.copyWith(swipeDown: v)),
                ),
                const Divider(height: 32),
                SwitchListTile(
                  title: const Text('Reverse page-turn direction'),
                  subtitle: const Text('For books that turn the other way'),
                ...
                Wrap(
                  spacing: 8,
                  runSpacing: 8,
                  children: PageTurnAnimationPreset.values.map((preset) {
                    return ChoiceChip(
                      label: Text(_animationLabel(preset)),
                      selected: current.animationPreset == preset,
                      onSelected: (_) =>
                          update(current.copyWith(animationPreset: preset)),
                    );
                  }).toList(),
                ),
                const Divider(height: 32),
...
String _gestureActionLabel(GestureMapAction action) {
  return switch (action) {
    GestureMapAction.showChrome => 'Show menu',
    GestureMapAction.enterDraw => 'Draw',
