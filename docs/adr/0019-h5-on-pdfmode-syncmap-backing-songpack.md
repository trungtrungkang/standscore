...
| 5 | MeasureMap: vẽ SystemBox → MeasureBox → BeatBox, chép layout trang trước; **nhảy tới ô nhịp** | 0056 | L | 1, 4 |
| 6 | SyncMap tính từ MeasureMap; metronome đi theo tempo và loại nhịp của bài | 0057 | M | 5 |
| 7 | Lật trang rảnh tay và chỉ báo vị trí, chạy bằng MeasureMap + SyncMap | 0058 | M | 6 |
| 8 | **Spike:** SoLoud 4.1.x, hai lane trên `getEngineTime`, đo độ trễ xuất âm trên máy thật | 0059 | spike | ADR này |
| 9 | Nhập BackingTrack + Transport (`ClickLane` nuốt metronome, `BackingLane`); sao lưu chọn lọc đi kèm | 0060 | L | 7, 8 |
| 10 | SyncMap gõ theo BackingTrack: chọn đơn vị gõ, làm trơn, sửa một điểm, offset hiệu chuẩn | 0061 | M | 9 |
| 11 | Định dạng SongPack: xuất, nhập có xem trước, siêu dữ liệu tác giả, cập nhật pack đã cài | 0062 | L | 10 |
| 12 | Màn hình SongPack | 0063 | M | 11 |
| 13 | **Spike:** đo độ lệch thu↔phát và chốt codec, một máy Android và một máy iOS | 0064 | spike | 9 |
| 14 | Thu âm từ microphone và line-in; SyncMap sinh theo cấu trúc | 0065 | L | 13 |
...
**Số Spec từ MeasureMap trở đi đã dịch ba bậc so với bảng gốc.** MeasureMap là **0056**, SongPack là **0062**, thu âm là **0065**. Các dòng cũ trong `DECISIONS-LOG.md` giữ nguyên số lúc viết — đọc theo bảng này.
...
- **Slices 1–3 (0052–0054) đã build; slice 4 là Spec 0055** (Score gốc chứa Score con). ADR này đã `accepted` ở G2; revision 6 chỉ viết lại quyết định 11 và bảng Trình tự — không mở lại toàn bộ G2 trừ khi Orchestrator muốn.
...
7. **`SystemBox`**, không phải `RowBox`. Đúng thuật ngữ khắc nhạc và khớp `systemIndex` bên repo web. Mọi chỗ trong ADR này còn viết `RowBox` đọc là `SystemBox`; sửa dứt điểm ở G1 của Spec **0056**.
