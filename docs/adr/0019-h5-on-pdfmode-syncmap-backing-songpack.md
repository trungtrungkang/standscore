...
- **BeatBox** — một phách, nằm trong một MeasureBox. Trên đĩa StageScore (Spec 0058 rev. 2): **N mốc nội tại** trong ô (không tính hai vạch nhịp mép) — mặc định tâm từng lát theo time signature; kéo từng mốc khớp nốt in. Web vẫn N−1 biên; dịch lúc trao đổi (quyết định 6).
...
Hai kênh độc lập, gặp nhau đúng một chỗ: playhead nội suy **vị trí** theo `beatSplits` (hình học), trong khoảng **thời gian** mà tempo / SyncMap quy định. SyncMap tính từ MeasureMap (**0059**) **không** đọc `beatSplits` để lấy thời lượng — chỉ tempo + time signature.

#### 3c. Một SyncMap sinh ra bằng một trong ba đường, và cả ba cùng đổ vào một cấu trúc

**Tính ra từ MeasureMap (+ FormMap).** Có tempo và time signature ở mỗi ô nhịp là có thời lượng mỗi lần thăm ô đó; cộng dồn theo **thứ tự nghe** — không phải luôn theo số ô in tăng dần. **FormMap** (Spec **0061**) mang lặp, volta, marker và nhảy (D.C. / D.S. / coda / Fine). FormMap **rỗng** ⇒ bung đồng nhất = nối mọi ô physical một lần (hành vi Spec **0059**). FormMap có dữ liệu ⇒ timeline **latent** (một ô physical có thể xuất hiện nhiều lần; đoạn bị nhảy cóc không có trên timeline), neo về physical để playhead vẽ đúng hộp — cùng họ web `TimemapEntry.measure` + `notationData.measureMap` (ADR 0014). **Không cần gõ một lần nào.** Đây là "default SyncMap tương tự midi syncmap" mà Orchestrator mô tả, và nó là mặc định — hễ MeasureMap tồn tại thì SyncMap tính ra tồn tại. MeasureMap vẫn chỉ là hình học; **cấm** nhồi repeat/coda vào MeasureBox như nguồn thời lượng (quyết định 3b).
...
`MeasureBox` **trùng tên và khái niệm** với repo web. Field `beatSplits` **trùng tên** nhưng **không còn trùng hình dạng store** sau Spec 0058 rev. 2 (G4 Orchestrator): StageScore = N mốc nội tại; web = N−1 biên. Đó không phải vi phạm ADR 0014 — không bịa tên thứ ba, không ép web đổi — miễn là phép dịch hai chiều ở quyết định 6 (và bảng 0014) ghi rõ centres ↔ midpoints.
...
| `beatSplits` — **N mốc nội tại** (vị trí từng phách) | `beatSplits` — **N−1 biên** giữa phách (dịch centres ↔ midpoints) |
...
Phép dịch phải chạy **cả hai chiều**. Với `beatSplits`, round-trip **đều** (mặc định tâm lát) không mất mát; kéo lệch hai phía có thể xấp xỉ qua midpoints — chấp nhận được vì thời lượng SyncMap không phụ thuộc field này. Nếu hai công cụ đẻ ra hai định dạng không dịch được cho nhau ở các hàng còn lại thì mọi MeasureMap soạn trong app sẽ vô dụng với danh mục, và ngược lại — và lúc đó thứ bị nhân đôi không còn là một màn hình mà là cả kho nội dung.
...
| 6 | SyncMap tính từ MeasureMap; **Playback controls** (play/pause/stop, show/hide) + **playhead** trên giấy | 0059 | M | 5 |
| 7 | **FormMap:** lặp / volta / D.C. / D.S. / coda → SyncMap **bung** (latent ↔ physical) | 0061 | L | 6 |
| 8 | Lật trang rảnh tay tinh — **`hold`** (Orchestrator skip tạm; làm sau FormMap) | 0060 | M | 7 |
| 9 | **Spike:** SoLoud 4.1.x, hai lane trên `getEngineTime`, đo độ trễ xuất âm trên máy thật | 0062 | spike | ADR này |
| 10 | Nhập BackingTrack + Transport (`ClickLane` nuốt metronome, `BackingLane`); sao lưu chọn lọc đi kèm | 0063 | L | 8, 9 |
| 11 | SyncMap gõ theo BackingTrack: chọn đơn vị gõ, làm trơn, sửa một điểm, offset hiệu chuẩn | 0064 | M | 10 |
| 12 | Định dạng SongPack: xuất, nhập có xem trước, siêu dữ liệu tác giả, cập nhật pack đã cài | 0065 | L | 11 |
| 13 | Màn hình SongPack | 0066 | M | 12 |
| 14 | **Spike:** đo độ lệch thu↔phát và chốt codec, một máy Android và một máy iOS | 0067 | spike | 10 |
| 15 | Thu âm từ microphone và line-in; SyncMap sinh theo cấu trúc | 0068 | L | 14 |
...
**Số Spec từ MeasureMap trở đi đã dịch năm bậc so với bảng gốc; revision 9 chèn FormMap và đẩy spike SoLoud+ thêm một bậc.** MeasureMap là **0058**, FormMap là **0061**, SongPack là **0065**, thu âm là **0068**. Spec **0060** giữ số nhưng `hold`. Các dòng cũ trong `DECISIONS-LOG.md` giữ nguyên số lúc viết — đọc theo bảng này.
...
Slice **1** chạy được song song với việc chờ v1 qua review: nó không chạm audio, không chạm mạng, không chạm tiền, nên nó không đụng gì tới bản đang nằm ở cửa store. Các slice offline đầu (MeasureMap → SyncMap → FormMap → lật trang rảnh tay) cũng vậy — **không có một byte nào rời khỏi máy và không thêm một SDK nào**, và chúng cộng lại đã là một sản phẩm đứng được: bản nhạc tách đúng bài, Library một hàng một tệp với drill-in, nhảy tới ô nhịp, metronome đi theo bài **đúng form** (lặp / coda), lật trang rảnh tay. Từ spike SoLoud / BackingTrack trở đi thì không còn chạy song song được.
...
- **Slices 1–4 (0052–0055) đã build; slice 5 Spec 0058 `accepted` ở G3 (2026-08-06), chưa build.** ADR này đã `accepted` ở G2; revision 6–7 chỉ đổi quyết định 11 / số Spec — không mở lại G2. Build MeasureMap thuộc chat riêng (ADR 0016).
